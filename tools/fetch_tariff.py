...
public Customs Trade Portal (customs.erca.gov.et) into tariff-data.js for the app.
    ...
    python3 tools/fetch_tariff.py            # full refresh (about 90 minutes)
    python3 tools/fetch_tariff.py --context  # only (re)add heading/subheading texts to tariff-data.js
...
The portal only gives each tariff line's own text (e.g. "--- Complete Built Up (CBU)"),
which is meaningful only under its heading and subheading. Those texts are the
international HS 2022 nomenclature (WCO), taken from the public-domain UN Comtrade
dataset. The run stops if any Ethiopian subheading has no HS 2022 text.
"""
import csv
import datetime
...
HS2022 = "https://raw.githubusercontent.com/datasets/harmonized-system/main/data/harmonized-system.csv"
...
def add_context(out):
    """Attach chapter, heading and subheading texts (HS 2022) for every code the tariff uses."""
    with urllib.request.urlopen(HS2022, timeout=120) as r:
        rows = {row["hscode"]: row for row in csv.DictReader(io.StringIO(r.read().decode("utf-8")))}
    need = {n: sorted({l["c"][:n] for l in out["lines"]}) for n in (2, 4, 6)}
    missing = [c for n in (2, 4, 6) for c in need[n] if c not in rows]
    if missing:
        sys.exit(f"STOP: HS 2022 has no text for {missing[:20]} ... Nothing written.")
    out["hs"] = {"source": "HS 2022 nomenclature (World Customs Organization) via UN Comtrade, public domain: " + HS2022,
                 "chapters": {c: rows[c]["description"] for c in need[2]},
                 "headings": {c: rows[c]["description"] for c in need[4]},
                 "subheadings": {c: rows[c]["description"] for c in need[6]}}
    print(f"HS 2022 texts: {len(need[2])} chapters, {len(need[4])} headings, {len(need[6])} subheadings", flush=True)
    return out
...
def main():
    if "--context" in sys.argv:
        text = open("tariff-data.js", encoding="utf-8").read()
        out = json.loads(text[text.index("{"):text.rindex("}") + 1])
        write(add_context(out))
        print("Updated tariff-data.js with heading and subheading texts")
        return
